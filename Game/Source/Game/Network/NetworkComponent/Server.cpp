...
void CServer::ProcessThePacket(const char* const BufOfPacket, const SOCKET& Socket)
{
	...
	{
		CONSOLE_LOG("[ERROR] <CServer::RegisterHeaderAndStaticFunc(...)> if (sizeOfRecvStream == 0) \n");
		return;
	}
	...
	{
		CONSOLE_LOG("[ERROR] <CServer::RegisterHeaderAndStaticFunc(...)> if (header >= MAX_HEADER || header < 0) \n");
		return;
	}
