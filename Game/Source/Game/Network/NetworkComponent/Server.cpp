...
#include "Console.h"
#//include "Clang/ClangPlatformAtomics.h" // For Interlocked, Atomics OS Functions (Apple?)
...
CServer::CServer()
{
	...
	while (InitializeCriticalSectionAndSpinCount(&csCountOfSend, SPIN_COUNT) == false);
...
void CServer::ProcessThePacket(const char* const BufOfPacket, const SOCKET& Socket)
{
	...
	{
		CONSOLE_LOG_NETWORK("[ERROR] <CServer::RegisterHeaderAndStaticFunc(...)> if (sizeOfRecvStream == 0) \n");
		return;
	}
	...
	{
		CONSOLE_LOG_NETWORK("[ERROR] <CServer::RegisterHeaderAndStaticFunc(...)> if (header >= MAX_HEADER || header < 0) \n");
		return;
	}
